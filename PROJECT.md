...
**masterにpushする前に必ず以下を順番に実行すること**

### ✅ Step 1: TypeScript型チェック + ビルドテスト
```bash
npm run build
```
...
**🚨 絶対ルール：Build/Lint/Test が通らないコードはmasterに入れない**
- GitHub Actionsで失敗 = デプロイ失敗 = ユーザーに届かない
- Lint失敗 = GitHub Actions の Test ワークフローが失敗
- Test失敗 = ロジックの不具合 = バグの混入
- **ローカルで事前チェックして品質を保つ**

**チェックリスト順序を守る理由:**
1. **Build** → 型エラーを最初に検出（最も基本的なチェック）
2. **Lint** → コーディング規約違反を検出（buildが通った後）
3. **Test** → ロジックの動作確認（最も時間がかかる）
