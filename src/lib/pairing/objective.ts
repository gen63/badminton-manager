 ...
 *
 * `AFFINITY_ENEMY_COST`（`computeAffinity`・大局評価）と同じ 1.0 に揃える案
 * （**この条件分けをせず一律に**上げる案）も検討したが、実測で明確に却下した。
 * 一律に上げると、同性希望ペアが2-2コートの両端実力にいる場面で「敵に分けて
 * mixSplit を守る」より「味方にして男女戦にする」ほうが安くなってしまい、
 * **男女戦（男男 vs 女女）を実際に増やす**（3-1・男女戦を避ける原則に反する）。
 *
 ...
 * 後退なので、「味方にすると男女戦になる」場合のこの値は一切動かさない。**
 *
