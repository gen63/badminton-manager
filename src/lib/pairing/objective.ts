...
/**
 * `assignRound.ts` の `splitCost`（チーム分けの**局所**決定）が使う
 * 「同コートで敵」の寄与（0〜1）。値は **0.5 のまま**（変更なし・却下した案）。
 *
 * `AFFINITY_ENEMY_COST`（`computeAffinity`・大局評価）と同じ 1.0 に揃える案も
 * 検討したが、実測で明確に却下した。この値を上げると、同性希望ペアが
 * 2-2コートの両端実力にいる場面で「敵に分けて mixSplit を守る」より
 * 「味方にして男女戦にする」ほうが安くなってしまい、**男女戦（男男 vs 女女）
 * を実際に増やす**（3-1・男女戦を避ける原則に反する。詳細は
 * `docs/plans/2026-08-31-pair-preference.md` 追記）。
 *
 * 実測（SEEDS=60・NOISE=0・同性希望ペア N=1・最悪ケース。男女戦%）:
 *
 * | 値 | 14人2C | 16人2C | 22人3C | 25人3C |
 * |---|---|---|---|---|
 * | 0.5（採用・変更なし） | 0.0 | 0.0 | 0.0 | 0.0 |
 * | 0.6 | 0.1 | 0.1 | 0.0 | 0.0 |
 * | 0.7 | 0.6 | 0.1 | 0.1 | 0.2 |
 * | 0.8 | 0.7 | 0.5 | 0.3 | 0.1 |
 * | 1.0 | 2.8 | 1.9 | 1.5 | 1.1 |
 *
 * 0.6 の時点で既に非0（ノイズではなく `MIX_SPLIT_WEIGHT` のコメントにある
 * 「競り度が mixSplit を上回る閾値 0.5〜0.6」に affinity 分が上乗せされて
 * 閾値を越え始めるため）。**「男女戦はほぼ0%」という原則に対して量に関わらず
 * 後退なので、この値は一切動かさない。**
 *
 * `scripts/bench-court-assignment.ts` の `AFFINITY_ENEMY_COST_SPLIT` 環境変数で
 * 個別に上書きして計測できる。
 */
export const AFFINITY_ENEMY_COST_SPLIT = { value: 0.5 };
 ...
 * ペアごとの寄与:
 *   味方（同コートで partner）  → 0
 *   同コートで敵                → AFFINITY_ENEMY_COST.value（既定 0.5）
 *   別コート / 片方以上がベンチ → 1.0
