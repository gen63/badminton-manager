...
describe('assignCourts - 3コート以上の逐次配置の実力分離 (hasTopBottomExtremes)', () => {
  ...
  it('21人3コート: 登録序列の上位×下位は原則として同居させない', () => {
    // 21人なので WIDE_EXTREME_BAND_MIN_ROSTER(=18) 以上 → band=3。
    // p20（最下位）を毎回勝たせてハシゴ式で上位グループへ押し上げる。
    //
    // **方針の変遷**:
    //   当初        : 順位差のハード制約を **登録レートの序列** で判定（許容3件）
    //   2026-08-18  : **ハシゴ式適用後の序列** に変更（許容22件 → skillGap 引き上げで30件）。
    //                 登録レートが実力より高すぎて負け続ける人を、実力相応の帯まで
    //                 降ろせるようにするため
    //   2026-08-25  : **両方の序列で判定**するように戻した（許容10件、実測8件）
    //
    // 最後の変更の理由: ハシゴ式後の序列だけで判定していると、登録レート的な
    // 最上位層と最下位層の同居が実データで **56.2%** まで起きていた
    // （2026-08-22 / 20人48試合）。運用上「初期レートの高い人が下位と混ざる試合は
    // 避けたい」という要求があり、登録序列側の判定を足した。
    //
    // 残る8件はフォールバック（制約を満たす組が枯れたときに配置を続ける経路）による。
    // ハシゴ式そのものを外しても 8 件までしか下がらないので、ここが実質的な下限。
    //
    // 代償の実測（20人3コート SEEDS=40 NOISE=3。登録上下% / 共演 / 3-1%）:
    //   なし 29.9% / 15.33 / 3.0%  →  2/3 併用 17.5% / 14.53 / 4.0%
