...
- `2026-09-08-recency-penalty.md` — 「試合数は揃うのに待ちが揃わない」問題への対処。実データ（20人3コート38試合）で試合数は6〜9（幅3）と均等なのに、空き11試合（≒25分）待つ人と空き1〜2で回り続ける人が混在していた。原因は配置の判断材料に**前回出場からの経過が一切無い**こと（`calculatePriorityScore` は試合数のみ、目的2 `waiting` は名前に反して「試合数順位が最上位の人を控えに回していないか」で経過時間を見ない、`lastPlayedAt` を使うのはシングルスの `SINGLES_WEIGHT_RECENCY`=500 だけ）。対策は優先度スコアではなく**目的関数に8つ目の項 `recency`（直前に出た人を弱く減点）**を足す形（優先度順位を動かさないので試合数の均等と遅参加のキャッチアップを壊さない。重みは `fairness`=1.5 未満が制約）。指標は時刻ではなく**試合履歴ベース**（`Date.now()` 依存だと bench が非決定的になり端末の時計差にも影響される）で `max(0, 1 − 最後の出場からの経過試合数 / コート数)` の配置者平均。重みは bench 実測で決定し、合格条件は 待ち 9.53→7.5以下 / 試合数幅 +0.2以内 / 占有率%・共演の悪化5%以内。最大のリスクは待機8人しかいない20人3コートで「残り4人」が確定して多様性が落ちること — 条件を超えて悪化するなら採用を見送る
