...
- `2026-08-13-auto-session-rating-sync.md` — 自動セッション再実行で tmp シートの skill を既存プレイヤーの rating へ反映（試合開始後は据え置き）+ 複でレート未設定の新規参加者の追加を保留。17:30 JST の定期実行（TARGET_DATE=nearest）を追加
