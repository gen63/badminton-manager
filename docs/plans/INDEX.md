...
- `2026-08-18-finish-operation-guide.md` — 「終了操作は次に入るメンバー（配置予測の濃い青）がする」運用を継続表示で明示。8秒で消える 4:30 トーストとは別に、`PresenceIndicator` の直下へ消えないガイドを置き、4:30 未満は「終了操作をお願いします」＋担当名、4:30 以降は「Nコート脇で終了操作をお願いします」に変える。コートは `callBasisCourtId`（空きコート優先）ではなく経過最大のプレイ中コート。判定は純粋関数 `finishOperationGuide.ts`、再描画は閾値までの `setTimeout` 1本。自分が担当のときの強調色は段階に合わせる（待機=濃い青 / 4:30以降=オレンジ）。併せて `PresenceIndicator` を小型チップ化（48px→28px、「たろうさんが操作中」→「たろう 操作中」）
