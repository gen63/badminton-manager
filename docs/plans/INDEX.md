...
- `2026-08-15-auto-session-sync-no-removal-after-start.md` — オートセッション再実行時の出欠同期で、セッション開始後（`hasSessionStarted`＝試合履歴あり or コートに配置済み）はメンバーを削除しない。ニックネーム表記揺れで手動追加した人や試合結果を持つ人を消してしまう事故を防ぐ。追加は従来どおり反映し、見送った削除は Discord に理由付きで通知。発生済みの事故は `scripts/repair-orphan-players.ts` で復旧（履歴の ID は残っているのでプレイヤーを作り直せば「未設定」は解消する）
