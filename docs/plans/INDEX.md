...
- `2026-08-31-pair-preference.md` — ペア希望（特定の2人が組む確率を上げる）。予約（ハード・使い捨て）とは別軸の恒常ソフト制約として、目的関数に第7項 `affinity` を追加する設計。評価対象は両者が候補プールにいる希望ペアのみ（実現不可能な希望を定数ペナルティとして残すと他項との重み比が薄まる）で、`affinity` は評価対象ペア数で平均するため希望1組のとき最大強度になり、重みはその最悪ケースで決める必要がある。落とし穴は `normalizeSplit` の `splitCost` が `competitive` + `mixSplit` しか見ていないこと — 目的関数側だけに足すと同コートに集まるが敵同士になる。順位差・直近重複・公平性の窓はハード制約（辞書式で先に評価）なので希望が破ることは構造上あり得ず、緩める分岐も作らない。ペア希望は試合に**入りやすくなる**方向に働くので、公平性リークには既存の `reservationBlockThreshold`（中央値+2）と同じガードを流用する（これが出場頻度への影響を抑える唯一の仕組み）。強度は `普通 / 必ず` の2段階で、違いは「`evaluate()` のハード制約（`StrongPair`）の有無」だけ：`必ず` は (a)「両方が出るなら必ず味方」に加えて (b)「2人一緒に出るか、2人とも控えるか」も課す。**2026-09-01 に2点の仕様変更を行った（詳細は文末の追記）**: (1) 当初は (a) のみ採用し「(b) は公平性の窓と衝突して詰む」としていたが誤りで、「控えが作れれば2人とも控えに回して両方の制約を満たせる」ことを確認して (b) も採用した。(2) `普通` の飽和（実績比率 `partner回数 / min(gamesPlayed)` から `TARGET_RATIO` 未達なら不足度に比例したペナルティを課し到達で0にする仕組み）を廃止し、対象ペアは常に最大強度で `variety` に対抗する（`AffinityPair` から `deficit` を削除）。同じ人を含む `必ず` の二重登録は UI で禁止。UI は予約ページに別セクション。予約と違い `isResting` にする副作用を持たせない。**2026-09-17 追記（実運用バグ修正）**: 同性の希望ペアが2-2コートで実力の両端に来ると `splitCost` が必ず敵に分けていたバグを特定・再現（`mixSplit` 重み1.0 が `affinity` の敵寄与0.5に勝つため）。「同コートで敵」の寄与を大局評価（`computeAffinity`／`AFFINITY_ENEMY_COST`）と局所のチーム分け（`splitCost`／`AFFINITY_ENEMY_COST_SPLIT`）で別定数に分離し、大局側だけ0.5→1.0に上げた（局所側を上げると男女戦が実際に増えるため却下・bench実測済み）。男女戦%を増やさず同居敵%をN=1で13〜28pt改善するが、コート構成に逃げ道が無い最悪ケース（ベンチ0等）は完全解消しないトレードオフとして残る。**続く追記（案C）**: 残存「同コートで敵」を内訳分析（味方にすると男女戦になる場合＝mixSplit由来／それ以外＝competitive由来）したところ、同性ペアは75〜100%がmixSplit由来（解消不能）だが異性ペアは100%がcompetitive由来（解消可能）と判明。「味方にしても男女戦にならない場合だけ」`splitCost`の敵寄与を強める第3定数`AFFINITY_ENEMY_COST_SPLIT_SAFE`（0.5→1.0）を追加し、男女戦%を一切増やさずに異性ペアの同居敵をほぼ解消、同性ペアも数pt追加改善。1.5・2.0はSEEDS=150で試合数幅が既存基準を超えたため1.0を採用。**2026-09-19 追記（運用者判断による仕様変更）**: 「男女戦を増やさない」を絶対条件から外し、「ペア希望が登録されているコートに限り男女戦を許容する」方針に変更。`AFFINITY_ENEMY_COST_SPLIT` を0.5→**1.2**（`mixSplit`重み1.0に確実に勝つ値。SEEDS=150で試合数幅+0.09に収め、1.4以上は基準超過のため不採用）に引き上げ、加えて `splitCost` の affinity 項から `affinityTargetCount`（登録組数）による除算を廃止（無関係コートの登録組数がこのコートの判断に影響する筋の悪い依存を解消。N=1/N=3で局所判断が完全一致することを確認）。希望ペアを含まないコートは affinity 項が加算されないため男女戦0.0%を維持（bench に希望コート/希望外コートを分けた男女戦%指標を追加して検証）。「案Cの対象外」としていた再現テスト（8人・ベンチ0・同性両端2-2）は `sameTeam=true` に更新
- `2026-09-01-bell-replay-last-match-call.md` — ヘッダーのベル OFF→ON 時の読み上げを、固定サンプル文言のテスト再生から**直前のコールの鳴らし直し**へ変更（手動の音声コール再実行）。直前のコールが無いときだけ従来どおりサンプル文言（`ゆーた` / 1コート）。`matchCallAlert.ts` にモジュールシングルトン `lastMatchCallSpeech` を持ち、`fireMatchCallAlert` の**設定 OFF・hidden の早期 return より前**で記録する（記録の意味は「コールが発生した内容」であって「鳴らせた内容」ではない。音を切っていて聞けなかったコールこそ鳴らし直したいため）。`speechText` なしの呼び出しでは記録を消さない。本人向け・管理者向けとも同じ入口を通るので記録先は1つ。永続化・有効期限は持たない（リロードで `null` に戻る／鳴るのはユーザーがベルを押した瞬間だけなので古いコールが勝手に鳴ることはない）。MainPage 側は文言決定を `buildBellSpeech(courtCount)` に切り出し、ベルやチャイム・振動の挙動は据え置き
- `2026-09-08-auto-end-stale-timer.md` — 「誰も押していないのに全コートの試合が終わる」「24〜27分というありえない試合時間が履歴に残る」バグの修正。原因は自動開始（3分）/自動終了（15分）の `setTimeout` が `Math.max(0, 期限 - now)` で遅延を出すため、**期限を過ぎている＝その間アプリが動いていなかった**ケースで復帰の瞬間に全コート分がいっせいに即発火すること（`finishedAt` が復帰時刻になるので試合時間も膨らむ）。方針は「自動処理はアプリが観測できていた時間だけを根拠にする」。タイマーの**発火の遅れ**（`AUTO_TIMER_MAX_LATENESS_MS`=2分。バックグラウンドのスロットリングは1分程度なのでその倍）で動いていた/止まっていたを判定する純粋関数 `isAutoEndDue` を追加し、`finishMatchAndContinue` に `autoEnd` オプションを足して transaction 内（＝どの端末から呼ばれても効く位置）で検証、遅れていれば書き込まず `stale_auto_end` を返して管理者にだけ手動終了を促す。自動開始も `assignedAt` が `ASSIGNED_AT_BASIS_MAX_AGE_MS`（3分+2分）より古ければ `null`（準備中のまま人に委ねる。古い時刻で開始すると即自動終了して幻の試合が記録される）。手動「開始」も同様に古い配置は `now` を `startedAt` にする（`resolveStartedAtFromAssignedAt`）。手動終了は従来どおり無条件
- `2026-09-08-settings-session-qr.md` — 設定画面の「セッションURL」カードに参加用 QR を常時インライン表示（`2026-08-18-settings-session-url-copy.md` の「QR は戻さない」を撤回）。URL コピーは送る手段がある相手にしか使えず、体育館でその場にいる人には画面を見せる方が速いため。エンコードは `qrcode.react`（v4・ランタイム依存ゼロ）の `QRCodeSVG` に任せ（Reed-Solomon とマスク選択を自前実装しない）、表示仕様は `SessionQrCode` コンポーネントに閉じ込める。URL は既存の `buildSessionUrl` をコピーボタンと共用。192px・誤り訂正 M・白背景/黒モジュールをハードコード（テーマ変数を使うとコントラストが崩れて読めなくなる）＋ `marginSize` でクワイエットゾーン確保。開閉トグルにはせず（緊急時にタップ段数を増やさない）、作成完了後の URL 画面・参加画面の共有アコーディオンは引き続き戻さない。**同日中に2点変更**: 初版は既存の `userIsAdmin` ブロック内に置いたため管理者限定かつ画面のかなり下だったが、(1) 権限による表示制御を解除して参加者全員に見せ（隣の人を誘うのは管理者に限らず誰でもやる。この画面に到達している時点で URL は既知なので新しい境界は作らない）、(2) 「この端末の設定」見出しより上＝コンテンツ先頭へ移動した（端末設定でもセッション設定でもなく「その場で人を呼ぶための情報」なのでどちらの見出しにも属さない）。さらに (3) 先頭に来たことで普段の設定変更のたびに 200px 弱の画像をスクロールで越える羽目になったため、QR だけを既定で閉じたアコーディオンに収納した（URL テキストとコピーボタンは常時表示のまま）。中身は条件レンダーで外さず `hidden` 属性で隠し、開いた瞬間の描画待ちを無くしつつ `aria-controls` の参照先を常に存在させる。開閉状態は永続化しない
- `2026-09-08-recency-penalty.md` — 「試合数は揃うのに待ちが揃わない」問題への対処。実データ(20人3コート38試合)で試合数は6〜9(幅3)と均等なのに、空き11試合(≒25分)待つ人と空き1〜2で回り続ける人が混在していた。原因は配置の判断材料に**前回出場からの経過が一切無い**こと(`calculatePriorityScore` は試合数のみ、目的2 `waiting` は名前に反して「試合数順位が最上位の人を控えに回していないか」で経過時間を見ない、`lastPlayedAt` を使うのはシングルスの `SINGLES_WEIGHT_RECENCY`=500 だけ)。対策は優先度スコアではなく**目的関数に8つ目の項 `recency`**を足す形(優先度順位を動かさないので試合数の均等と遅参加のキャッチアップを壊さない)。1回目は「直前に出た人を弱く減点」(gapベース)で bench 実測したが、bench が配置時に履歴を積んでいて直前組の gap が正しく0にならない不備が見つかり計測をやり直した。修正後の bench で改めて重みを振っても、事前に置いた合格条件(待ち9.53→7.5以下)には届かず一旦見送り。ユーザー提案で「2連続までは自由、3連続目から嫌う」**streak ベース**に作り替え(`RECENCY_STREAK_SHAPE`: allowance=2/ramp=2)、bench に主指標「3連続%」を追加して再計測したが、この形は後追いになるうえ配置12人平均に薄まり全条件で現状と有意差なし。「1回目から連続が長いほど強く」(`allowance=0`)に変えると3連続%が2〜3割改善し副作用も許容内(占有率/共演の悪化5%以内・遅参加のキャッチアップ無傷・3-1%±1pt程度)だったため**採用**(`RECENCY_STREAK_SHAPE.allowance=0` / `RECENCY_WEIGHT=1.2`)。1回の再出場から弱く(0.5)・2連続以上で満点(1.0)
- `2026-09-16-global-default-fees.md` — 練習種別ごとの基本参加費（`PRACTICE_TYPE_OPTIONS`）がコード定数でデプロイしないと変えられない問題への対処。既存の `appConfig/global`（デフォルト周知事項の置き場）に `defaultFees` を相乗りさせ、開発モードのセッション一覧から編集できるようにする（新コレクション・新 Security Rules 不要、UI も周知事項の編集ブロックと同型）。会費の解決順は `session.accounting` > グローバル既定 > 同種別の直近履歴 > コード定数。グローバル既定を履歴継承より上に置くのは、管理者が明示的に決めた値がたまたま残っていた前回入力に負けるのは直感に反するため。未設定なら解決順は現行と完全一致（後方互換）。純粋関数 `resolveFees(practiceType, overrides?)` に解決を集約し `calculateAppropriateFee` の男女差もこれ経由にして既定変更に追従させる。読み取りは `fetchDefaultFeesSafe()`（失敗を warn に落として `{}`）＋ `useDefaultFees` のモジュールレベル Promise キャッシュ（保存時に invalidate）で、会計画面が開けなくなるのと画面遷移ごとの再読込を防ぐ。**既存セッションには遡及しない**（`sessions/{id}.accounting` の実績値を後から書き換えないための意図的な制約）。権限制御は増やさない（認証が無いので `?dev=232` を知っていれば誰でも変更できる。全体デフォルトをクライアントから書ける以上避けられず受け入れる）
- `2026-09-19-match-result-input-mode.md` — 試合結果の登録方式を「簡易（勝敗のみ）」「詳細（点数入力）」から**個人ごと（端末ごと）**に選べるようにする。新設定 `matchResultInputMode: 'simple' | 'score'`（既定 `'simple'`）は `settingsStore` の `partialize` に入れて端末ローカル persist し、Firestore には同期しない（`finishHoldToConfirm` と同じ理由＝1人の好みで全員の入力方式を変えてはいけない）。セッション共有の `recordScores`（結果を記録するか否か）とは独立した別軸で、`recordScores` OFF のときは従来どおりプロンプト自体を出さない。分岐点は `UnrecordedMatchPrompt` の1箇所のみ: `'simple'` は現行どおり `WinnerSelectModal` + 100-99 のダミースコア（既定なので既存ユーザーの挙動は完全に不変）、`'score'` はモーダルを出さず `/score/:matchId` へ `navigate`（`state.from='/main'` で戻り先をメイン画面に）。遷移直前に既存のスヌーズ機構 `unrecordedDismissStore.dismiss`（10分）で当該 matchId を抑制し、入力せず戻ったときの再発火ループを防ぐ（入力して戻れば scoreA/scoreB が入り対象から外れる）。却下案は **点数入力のモーダル化**（`ScoreInputPage` はページ前提のレイアウトと `useParams`/`useNavigate`・削除/離脱導線を持ち、モーダル化は実装二重化かページ大改修になる。入力方式選択という小さな目的に見合わないので既存ページをそのまま再利用）
