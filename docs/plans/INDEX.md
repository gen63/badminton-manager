...
- `2026-08-31-pair-preference.md` — ペア希望（特定の2人が組む確率を上げる）。予約（ハード・使い捨て）とは別軸の恒常ソフト制約として、目的関数に第7項 `affinity` を追加する設計。「必ず一緒ではないが確率を上げる」は乱数ではなく**飽和**で表現する（実績比率 `partner回数 / min(gamesPlayed)` が `targetRatio` を下回っている間だけ不足度に比例したペナルティを課し、到達したら 0 になって `variety` が散らす）。評価対象は両者が候補プールにいる希望ペアのみ（実現不可能な希望を定数ペナルティとして残すと他項との重み比が薄まる）。落とし穴は `normalizeSplit` の `splitCost` が `competitive` + `mixSplit` しか見ていないこと — 目的関数側だけに足すと同コートに集まるが敵同士になる。順位差・直近重複・公平性の窓はハード制約（辞書式で先に評価）なので希望が破ることは構造上あり得ず、緩める分岐も作らない。実運用レンジ（3コート最大25人・通常22人）ではハード制約はほぼ当たらない（`ceil(人数×2/3)`・14人未満はオフ。22人でもブロックは1位×16位以降だけ）ため、UI 警告は1グループ幅 `ceil(人数/3)` を基準にする。ペア希望は試合に**入りやすくなる**方向に働くので、公平性リークには既存の `reservationBlockThreshold`（中央値+2）と同じガードを流用する。平均正規化により総影響量は組数によらず一定だが、1組あたりの効き目は 1/N に薄まり公平性の偏りは1組のときが最大（推奨1〜3組）。`affinity` は評価対象ペア数で平均するため希望1組のとき最大強度になり、重みはその最悪ケースで決める必要がある。強度は `普通 / 必ず` の2段階。**`targetRatio` を 1.0 にしても「必ず」にはならない**（有限の重みは他項の合計に押し負けうる）ため、`必ず` はハード制約側に置く。意味論は「両方が同じラウンドで出るなら必ず味方」だけに絞る（「片方が出るならもう片方も」は公平性の窓と衝突して挙動が予測不能になる）。この形なら常に充足可能で詰まず、出場頻度に干渉しないので公平性の設計も変わらない。同じ人を含む `必ず` の二重登録は UI で禁止。UI は予約ページに別セクション。予約と違い `isResting` にする副作用を持たせない
