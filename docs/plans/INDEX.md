...
- `2026-08-21-session-list-average-rating.md` — セッション一覧カード1行目の収入合計（💵）を平均レート表示に置き換え（開発モード限定）。母集団は名簿上の `rating > 0` のプレイヤー（未レートの 0 は「実力不明」なので除外、試合数では絞らないので開催前でも出る）。統計量は中央値ではなく平均（2行目の「中央」＝試合数中央値と紛れないため）。一覧専用の `computeDerivedIncomeTotal` と `Session.incomeTotal` は削除し、会計ページ側の金額計算は据え置き
