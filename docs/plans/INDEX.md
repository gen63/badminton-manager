...
- `2026-08-18-finish-operation-guide.md` — 「終了操作は次に入るメンバー（配置予測の濃い青）がする」運用を継続表示で明示。8秒で消える 4:30 トーストとは別に、`PresenceIndicator` の直下へ消えないガイドを置き、担当の周知は配置予測バーの見出し「配置予測（操作担当）」が担い、上部ガイドは4:30以降に「②付近待機 操作担当」として出す（4:30未満は同じ顔ぶれが上下2箇所に出て冗長だったため廃止）。依頼文ではなく役割ラベル・コート番号は丸数字で1行に収める。期待する役割が終了→配置→開始の一連なので「終了操作担当」には狭めない。コートは `callBasisCourtId`（空きコート優先）ではなく経過最大のプレイ中コート。判定は純粋関数 `finishOperationGuide.ts`、再描画は閾値までの `setTimeout` 1本。自分が担当のときの強調色は段階に合わせる（待機=濃い青 / 4:30以降=オレンジ）。併せて `PresenceIndicator` を小型チップ化（48px→28px、「たろうさんが操作中」→「たろう 操作中」）。継続表示と完全に重複する4:30/5:00のトースト（本人向け・管理者向けとも）は廃止し、OS通知・音・振動・読み上げのみ残す。代わりに閾値を跨いだ瞬間だけガイドを明滅
