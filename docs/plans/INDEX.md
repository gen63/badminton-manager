...
- `2026-08-22-standby-guide-always-on.md` — 上部の待機ガイドを「4:30 以降のみ」から**プレイ中コートがある間は常時表示**へ戻し、待機場所を「近接コートの集合」から決める。経過最大コートとの差が60秒以内（`STANDBY_CLOSE_START_MS`）のプレイ中コートをまとめ、2面までなら `①②付近待機`、3面以上（同時開始）と1面運用は番号を落として `コート付近待機`。待機場所は 4:30 を待たずに分かるので早めに移動できる利を取る一方、3面同時開始の「①②③で待機」は場所の案内にならないため出さない。`FinishOperationGuide` は `courtId: number \| null` → `courtIds: number[]`、`FinishGuidePhase`（`waiting`/`imminent`）を復活させ、4:30 未満は控えめな地色・4:30 以降はオレンジ＋跨いだ瞬間だけ明滅（自分が担当のリング/チップは待機段階は配置予測と同じ indigo）。判定タイマーは段階切り替えの1本のみ（近接判定は経過差ベースで時間が経っても変わらない）。OS通知・読み上げの文言は据え置き
