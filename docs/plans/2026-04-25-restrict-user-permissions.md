...
> **注記 (2026-05-20)**: 本文中の「ローカルモード」「session.createdBy が無いとき
> isAdmin() が常に true」は Phase 4 クリーンアップで撤廃された。現在 `isAdmin()`
> は dev モード以外では「session 有り + currentUser がセッション作成者/admins」
> のときのみ true。詳細は `docs/plans/2026-05-20-phase4-cleanup.md`。
