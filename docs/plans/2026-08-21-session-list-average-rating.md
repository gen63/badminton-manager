...
値）。その日の勝敗から解く `performanceRating`（1500 基準のパフォーマンスレート）では
ない。**スケールは 1500 基準ではなく運用側の入力次第**で、実運用のロースターは 16〜40
程度・小数2桁（未入力の人も混じる）。

### 統計量

**平均**（中央値ではなく）。初期レートは連続値（小数2桁）で外れ値が飛び抜けにくく、
面子の層を1つの数字で表すなら平均が素直（`performanceRating` の「相手平均 / 味方平均」も
平均を採っている）。2行目の試合数チップが既に「中央」なので、同じラベルが並んで意味を
取り違えるのも避けられる。
...
小数第1位で丸める（`summarizeSessionMedians` と同じ丸め）。実運用のレートは 16〜40 程度・
小数2桁なので、整数へ丸めると開催間の差が潰れる。逆にスケールは運用側の入力次第なので、
1500 基準を仮定した表示（整数固定・3桁区切り）もしない。
...
```
[複] 8/21(木) 14名 千川館 32試合 ⏲平均27.4     ← 1行目（💵 の枠を置き換え）
