...
4. **最後にコート横断重複除去（dedup）** を実施:
   ...
   - 重複を検出した場合、**そのコートが local 側で base から変更されたか** を判定:
     - **local-changed のコートに残す**（ユーザの直近操作を保持）
     - 他方のコート上の該当ポジションを `''`（空文字）に置換
   - 両方とも local-changed の場合（ありえないが防御的に）: 数値ID が小さい方のコートに残す。
