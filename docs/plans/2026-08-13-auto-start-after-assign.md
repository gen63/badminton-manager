...
この変更で `startedAt` は「試合の経過時間の起点」に専念し、**操作が起きた瞬間**とは
ずれるようになった。操作からの経過で判断していた終了ボタンのロックが効かなくなるため、
`Court.startPressedAt`（「開始」の操作時刻）を追加して切り分ける。

- `startPressedAt` に入るのは、手動「開始」を押した時刻 / 自動開始が走った時刻 /
  一括配置で即開始した時刻。表示には使わず、誤タップ防止の判断にだけ使う。
- `FinishGameButton` のロックは `startedAt` ではなく `startPressedAt` 起点にし、
  ロック時間は 10 秒 → **5 秒**に短縮。
- 旧データ（`startPressedAt` なし）は `startedAt` にフォールバックする。

「1分以内の終了は確認ダイアログ」（`2026-05-30-quick-finish-confirmation.md`）は
`startedAt` 起点のまま。あちらは誤タップ防止ではなく「試合として短すぎないか」の
確認なので、経過時間で判断するのが妥当と考えた。
