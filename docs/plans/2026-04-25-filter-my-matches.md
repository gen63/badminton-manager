...
> **注記 (2026-05-20)**: 本文中の「ローカルモード（session.createdBy なし）の
> ときフィルタを非表示」は Phase 4 クリーンアップで撤廃された。現在は
> `!!session && !!currentUser` のときのみフィルタを表示する。詳細は
> `docs/plans/2026-05-20-phase4-cleanup.md`。
