...
セッション作成画面で、Google スプレッドシートからメンバー一覧を読み込み、当日の練習参加者としてセットする機能。
...
## スプレッドシートの「メンバー」シート構成

ユーザーが「メンバー」という名前のシートを作成し、以下の形式でメンバーを記入。
...
2. GAS経由でスプレッドシートの「メンバー」シートからデータを取得
3. 取得したメンバーをテキストエリアに反映（既存入力は上書き）
4. ユーザーが不参加者を手動で削除・追加メンバーを記入
5. 「開始」でセッション作成

**ポイント:**
- 既存のテキストエリア入力パターンをそのまま活用
- 読み込み後も自由に編集可能
- 選択UIは不要（全員読み込み→不要な人を消す方がシンプル）
...
// 新規追加: メンバー読み込み用
function doGet(e) {
  var sheetName = (e && e.parameter && e.parameter.sheet) || 'メンバー';
...
| シート名 | GAS側で「メンバー」固定（クエリパラメータで変更可能） |
...
| 全員読み込み→手動削除 | メンバー選択UIを作るより圧倒的にシンプル |
| シート名「メンバー」固定 | 設定を増やさない。GAS側で変更可能 |
...
1. スプレッドシートに「メンバー」シートを作成
2. A列に名前、B列にレーティング（任意）を記入
3. GASスクリプトを更新（doGet追加）して再デプロイ
4. アプリのセッション作成画面で「Sheetsから読み込み」をタップ
