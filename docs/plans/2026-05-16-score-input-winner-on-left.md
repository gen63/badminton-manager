...
- `inputHistory` の初期化（編集モード）も左右順で `[leftScore, rightScore]`
  に統一する。
...
- 履歴で teamA 勝ち試合の編集 → 表示そのまま (teamA 左)
- 履歴で teamB 勝ち試合の編集 → 表示が左右入れ替わり、左に teamB の
  プレイヤーとスコアが表示される
- 編集モードでクリアして再入力 → 最初のタップが左 (winner) のスコア、
  次のタップが右 (loser) のスコアになる
- 未入力試合の編集 → 従来通り teamA 左
