...
2. `applyStreakSwaps`: 勝敗に基づき序列を調整
3. 3等分（端数はmiddleに）
...
### applyStreakSwaps の動作

| イベント | 移動幅 |
|---------|-------|
| 勝利（通常） | 1つ上に移動 |
| 2連勝ごと（2, 4, 6...） | グループ1つ分（`floor(人数/グループ数)`）上に移動 |
| 敗北 | 移動なし（勝者の昇格により自然に繰り下がる） |

```
例: 12人、3コート（groupSize = 4）
  12番が1勝目 → 1つ上（11番の位置）に移動
  12番が2勝目(2連勝) → 4つ上（7番の位置）に移動 → middle入り
  12番が3勝目 → 1つ上（6番の位置）に移動
  12番が4勝目(4連勝) → 4つ上（2番の位置）に移動 → upper入り
```
