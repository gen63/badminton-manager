...
- 名前の先頭に `外部`（ゲスト参加者を表す運用上のラベル）が付いている場合、
  読み上げ時のみそれを除去する。除去対象は `SPEECH_IGNORED_NAME_PREFIXES`
  定数配列で管理し、将来 `ゲスト` 等を増やす場合は配列に足すだけでよい形にする。
  先頭一致を1回だけ除去すれば十分とし、繰り返し（`外部外部たろう` 等）は考慮しない。
- 処理順序は「記号除去 → 接頭辞除去」で固定する。先に記号を除去することで、
  `【外部】はなこ` のような括弧付き表記も `外部はなこ` に正規化されてから
  接頭辞が落ち `はなこ` になる。逆順だと括弧が接頭辞の前に残り除去できない。
- この接頭辞除去も表示（body / toast）には影響させない。読み上げ専用の加工である。
...
- `fireMatchCallAlert(speechText?: string): void` — 既存のチャイム・振動に加え、
  ...
  チャイム（0.3 秒）の末尾とわずかに重なるが、間延びを避けるため `setTimeout` で 200ms 遅らせる。
...
- `fireMatchCallAlert` / ベルのテスト再生は、いずれも `speak()` の呼び出し自体を
  `setTimeout` で 200ms 遅らせている。そのため読み上げが実際に始まる時点では、
