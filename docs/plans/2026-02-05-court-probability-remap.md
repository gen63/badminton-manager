...
- レーティングが0のプレイヤーは、**初期序列で middle の位置**に配置する
- 連勝/連敗による序列入れ替えの対象になる（他のプレイヤーと同様に昇降格する）
...
初期序列: A > B > C > X > D > E
                      ↑ レーティング0は middle の位置に挿入

グループ分け（6人の場合）:
  [upper]  A, B        ← 序列上位
  [middle] C, X        ← 序列中位（Xは初期 middle）
  [lower]  D, E        ← 序列下位
