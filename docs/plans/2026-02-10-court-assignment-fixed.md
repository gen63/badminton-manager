...
**「グループ上位は2敗で降格、下位は1敗で降格」** を実現するdrop量:
...
※ 上記はグループ最上位(pos 0)からの連敗数。グループ下位は1敗で境界を越える。

#### 具体例（21人、groupSize=7、drop=4）

```
Upper (pos 0-6)  |  Middle (pos 7-13)  |  Lower (pos 14-20)
```

| 位置 | 1敗後 | 結果 |
|------|-------|------|
| Upper #1 (pos 0) | pos 4 | Upper内残留 → **2敗目で降格** |
| Upper #2 (pos 1) | pos 5 | Upper内残留 → **2敗目で降格** |
| Upper #3 (pos 2) | pos 6 | Upper内残留 → **2敗目で降格** |
| Upper #4 (pos 3) | pos 7 | **1敗で降格（Middle）** |
| Upper #5 (pos 4) | pos 8 | **1敗で降格（Middle）** |
| Upper #6 (pos 5) | pos 9 | **1敗で降格（Middle）** |
| Upper #7 (pos 6) | pos 10 | **1敗で降格（Middle）** |

→ **上位3人は2敗で降格、下位4人は1敗で即降格。** これは意図した設計:
- グループ上位 = そのレベル帯の実力者 → 1敗では猶予あり
- グループ下位 = 境界付近 → 1敗で移動は自然（もともとギリギリのライン）
