...
{isCreator && (
  <>
    <button>練習をリセット</button>
    <button>履歴をコピー</button>
  </>
)}
...
#### Firestore データ構造（Phase 2）
```typescript
// sessions/{sessionId}
{
  createdBy: "管理者の名前",
  subAdmins: ["サブ1", "サブ2"],  // サブ管理者リスト
  participants: ["太郎", "花子", ...]
}

// 権限判定
function getRole(userName, session) {
  if (userName === session.createdBy) return 'admin';
  if (session.subAdmins.includes(userName)) return 'subAdmin';
  return 'participant';
}
```
