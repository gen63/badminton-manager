...
**注記 (Phase 0)**:
- 外部連携（スプレッドシートURL）およびURL共有機能はPhase 0では非表示。
- セッションはLocalStorageに保存され、同一端末内でのみ有効。
...
| S03-12a | セクション | スコア未入力の試合 | スコア未入力の試合一覧（**最大2件**表示） |
...
**注記 (Phase 0)**:
- 外部データ連携（スプレッドシート）の設定はPhase 0では非表示。
