...
Tailwind CSSの設計哲学に基づいた一貫した余白の管理です。

| 用途 | 値 | Tailwind | 例 |
|------|-----|----------|-----|
| **カード内パディング** | 16px | `p-4` | カード全体の内側余白 |
| **セクション間** | 24px | `space-y-6` / `mb-6` | カード間、大きなブロック間 |
| **ラベル↔入力** | 8px | `mb-2` | ラベルと入力フィールドの間 |
| **要素間（小）** | 8px | `gap-2` | 近い関係の要素 |
| **要素間（中）** | 12px | `gap-3` | 標準的な要素間 |
| **要素間（大）** | 16px | `gap-4` | 独立した要素間 |
